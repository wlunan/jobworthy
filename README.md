...
- **学历字段下线**：招聘机会页不再展示、筛选（搜索字段/学历下拉）、导出「学历」列；旧数据中残留的 `e` 字段会被忽略
- **数据源筛选（预留）**：每条岗位记录新增 `ds` 数据源标识，当前默认均为 `"校招信息聚合平台"`；工具栏新增「数据源」下拉，暂只内置默认数据源，后续接入其他表格岗位数据时追加选项即可
- **指定字段搜索**：搜索框内置字段选择器（全部字段/公司/职位/地点/行业/批次/来源），避免多字段模糊搜索时无法区分命中字段
...
### Q: 页面里的数据是实时的吗？
A: 是。页面打开时自动读取服务器上每日同步的 `jobs.json`，无需手动导入导出。
