...
- **数据每日同步**：`scripts/sync_tencent_docs.py` 每日自动从多份腾讯文档智能表格（默认聚合表 + 27届秋招内推优先投表）拉取最新数据生成 `jobs.json`，页面打开即读
...
详细数据量与覆盖以 `jobs.json` 中 `count` 字段为准。`jobs.json` 目前合并自两份腾讯文档智能表格：
- **校招信息聚合平台**（27届实习提前批信息汇总）：以默认聚合表为主，含企业官网/公众号校招节点；
- **27届秋招内推（优先投）**（27届秋招含提前批内推-日更）：秋招提前批/正式批内推渠道，记录在页面「数据源」下拉中可单独筛选查看。
